...
- 🧬 **遗传算法组卷** — 支持自动组卷，可按难度分布、题型分布、必选标签等约束优化
- ⚖️ **分值权重** — 试题支持 `scoreWeight` 字段，遗传算法组卷时结合权重分配分值
...
| `teacher` | `teacher123` | 教师 — 可创建、编辑题目和试卷，不可删除试题 |
...
| 首页 | `/` | 否 | — | 功能概览，快捷入口 |
| 登录 | `/login` | 否 | — | 用户登录 |
| 注册 | `/register` | 否 | — | 新用户自助注册（默认 viewer 角色） |
